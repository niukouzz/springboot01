...
// 本试卷唯一 ID: F8BC78BBAA44F982092A677B034D5036, 请勿修改本行内容
/* 注意, 这里不要写包名. 留空 */
...
//            list.add(nums.)
//        }
        int i = nums.length;//行数
        int j = nums[1].length;//列数
        int row = 0; // 行号
        int col = j - 1; // 列号
        //判断在此范围
        while (row >= 0 && row < i && col >= 0 && col < j) {
            if (nums[row][col] == x) { // 如果找到了就直接退出
                return true;
            } else if (nums[row][col] > x) { // 如果找到的数比要找的数大，说明要找的数在当前数的左边
                col--; // 列数减一，代表向左移动
...
//        });
    ...
    static boolean isSymmetry(TreeNode t1, TreeNode t2) {
        if(t1 == null && t2 == null){
            return true;
        }
        if(t1 == null || t2 == null){
